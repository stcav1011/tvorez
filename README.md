...
Работает с YouTube, SoundCloud, Instagram, TikTok, VK, Vimeo и ещё ~1800 сайтами
(внутри — [yt-dlp](https://github.com/yt-dlp/yt-dlp) и ffmpeg).

## Что умеет

- **Три режима:** видео со звуком (MP4), только видео без звука, только звук.
- **Формат звука:** WAV без потерь для монтажа или «как в источнике» (MP3/M4A) —
  трек с SoundCloud весит около 5 МБ вместо 45 МБ. Если Resolve не читает исходный
  кодек (opus, vorbis), скрипт сам переводит звук в AAC.
- **Ссылки без видео** (SoundCloud, Bandcamp, Mixcloud) сами переключают режим на «только звук».
...
- **Понятные ошибки:** вместо сырого вывода yt-dlp пишет, что делать.
...
- Для Instagram, закрытых и 18+ видео выбери браузер в строке **«Браузер»**:
  ...
  это нормально. Для YouTube и SoundCloud вход обычно не нужен.
- **Safari** отдаёт cookies только приложению с полным доступом к диску:
  Системные настройки → Конфиденциальность и безопасность → Полный доступ к диску →
  добавь DaVinci Resolve и перезапусти его. Проще выбрать Chrome.
- **DRM-треки** (например, SoundCloud Go+) скачать нельзя — это защита на стороне сайта.
